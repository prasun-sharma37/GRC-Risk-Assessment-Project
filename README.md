...
AWS Cloud EHR Database: Primary repository for all Patient Health Information (PHI).

Remote Employee Endpoints: Laptops issued to doctors, nurses, and administrative staff.

Telehealth Video Application: The proprietary web portal used for doctor-patient consultations.

AWS S3 Cloud Storage: Used for storing patient documents, lab results, and backups.
...
Cybercriminal Group (Ransomware)

Lack of robust database backups and network segmentation.
...
Implement Immutable Backups, Network Segmentation, and EDR on database servers.
...
Opportunistic Thief (Physical Theft)

Missing Full Disk Encryption (FDE); weak password policies.
...
Enforce BitLocker/FileVault via MDM; Implement strong password policies and remote wipe capabilities.
...
Script Kiddie / Malicious Hacker

Lack of rate limiting on login portal; no Multi-Factor Authentication (MFA).
...
Enforce mandatory MFA for all users; Implement Web Application Firewall (WAF) to block brute-force.
...
Cloud misconfiguration (S3 bucket left public without IAM restrictions).
...
Automated Cloud Security Posture Management (CSPM) alerts; strict IAM Least Privilege enforcement.
...
Advanced Persistent Threat / Cybercriminal

Employee falls for targeted phishing email delivering malware.
...
Monthly Security Awareness Training, Phishing Simulations, and Email Gateway Filtering.
